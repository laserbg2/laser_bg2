...
struct usb_device_id driver_usb_table[] = {
    ...
    {MARVELL_USB_DEVICE(MARVELL_USB_VID, MARVELL_USB8766_PID, "Marvell WLAN USB Adapter")},
    {MARVELL_USB_DEVICE(MARVELL_USB_VID, MARVELL_USB8797_PID, "Marvell WLAN USB Adapter")},
    {MARVELL_USB_DEVICE(MARVELL_USB_VID, MARVELL_USB8897_PID, "Marvell WLAN USB Adapter")},
    /* Terminating entry */
    {},
};
...
driver_prog_fw_w_helper(IN driver_handle *handle, IN pdriver_fw_image pmfw)
{
    ...
    ret =
        driver_malloc(handle, FW_DNLD_TX_BUF_SIZE,
                         DRIVER_MEM_DEF | DRIVER_MEM_DMA, (t_u8 **) & fwdata);
    ...
    ret =
        driver_malloc(handle, FW_DNLD_RX_BUF_SIZE,
    ...
    do {
        ...
        if (check_winner) {
            ...
            if (firmware)
                ...
                driver_get_fw_data(handle, TotalBytes, sizeof(FWHeader),
                                      (t_u8 *) & fwdata->fw_header);

            DataLength = driver_le32_to_cpu(fwdata->fw_header.data_length);
            DnldCmd = driver_le32_to_cpu(fwdata->fw_header.dnld_cmd);
            ...
            if (firmware)
                ...
                driver_get_fw_data(handle, TotalBytes, DataLength,
                                      (t_u8 *) fwdata->data);
        ...
        while (retries) {
            ...
            if ((ret = driver_write_data_sync(handle, &mbuf, DRIVER_USB_EP_CMD_EVENT,
            ...
            if ((ret = driver_read_data_sync(handle, &mbuf, DRIVER_USB_EP_CMD_EVENT,
                                                DRIVER_USB_BULK_MSG_TIMEOUT)) !=
                DRIVER_STATUS_SUCCESS) {
...
driver_usb_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
    ...
    udev->descriptor.idVendor = driver_cpu_to_le16(udev->descriptor.idVendor);
    udev->descriptor.idProduct = driver_cpu_to_le16(udev->descriptor.idProduct);
    udev->descriptor.bcdDevice = driver_cpu_to_le16(udev->descriptor.bcdDevice);
        ...
        for (i = 0; i < iface_desc->desc.bNumEndpoints; ++i) {
            ...
            if ((endpoint->bEndpointAddress & USB_ENDPOINT_DIR_MASK) &&
                ...
                PRINTM(MINFO, "Bulk IN: max packet size = %d, address = %d\n",
                       driver_le16_to_cpu(endpoint->wMaxPacketSize),
                                     ...
                                     t_u32 *usb_chip_rev)
{
    ...
    /* Allocate memory for transmit */
    if ((tx_buff = kzalloc(CHIP_REV_TX_BUF_SIZE, GFP_ATOMIC | GFP_DMA)) == NULL) {
        PRINTM(MERROR, "Could not allocate buffer for chip revision check frame transmission\n");
        ret = DRIVER_STATUS_FAILURE;
        goto cleanup;
    }

    /* Allocate memory for receive */
    if ((recv_buff = kzalloc(CHIP_REV_RX_BUF_SIZE, GFP_ATOMIC | GFP_DMA)) == NULL) {
        PRINTM(MERROR, "Could not allocate buffer for chip revision check frame response\n");
        ret = DRIVER_STATUS_FAILURE;
        goto cleanup;
    }
    ...
    if ((ret = driver_write_data_sync(handle,
       &mbuf, DRIVER_USB_EP_CMD_EVENT, DRIVER_USB_BULK_MSG_TIMEOUT)) != DRIVER_STATUS_SUCCESS) {
       PRINTM(MERROR, "Chip revision check frame dnld: write_data failed, ret %d\n", ret);
       ret = DRIVER_STATUS_FAILURE;
       goto cleanup;
    }
    ...
    if ((ret = driver_read_data_sync(handle,
       &mbuf, DRIVER_USB_EP_CMD_EVENT, DRIVER_USB_BULK_MSG_TIMEOUT)) != DRIVER_STATUS_SUCCESS) {
       PRINTM(MERROR, "Chip revision check frame response: read_data failed, ret %d\n", ret);
       ret = DRIVER_STATUS_FAILURE;
       goto cleanup;
    }
...
driver_update_fw_name(driver_handle * handle)
{
    ...
    if (fw_name) {
        ...
        if (handle->usb_dev_type == DEV_TYPE_USB8766) {
            ...
            if ((ret = driver_check_chip_revision(handle, &revision_id)) != DRIVER_STATUS_SUCCESS) {
                PRINTM(MFATAL, "USB8797 Chip revision check failure!\n");
                ret = DRIVER_STATUS_FAILURE;
            } else {
                /* Check revision ID */
                switch (revision_id) {
                    case USB8797_A0:
                        handle->fw_name = USB8797_A0_DEFAULT_FW_NAME;
                        break;
                    case USB8797_B0:
                        handle->fw_name = USB8797_B0_DEFAULT_FW_NAME;
                        break;
                    default:
                        break;
                }
...
driver_init_fw_dpc(driver_handle * handle)
{
    ...
    if (handle->firmware) {
        ...
        if (ret != DRIVER_STATUS_SUCCESS) {
            PRINTM(MERROR, "WLAN: Download FW with nowait: %d failed. ret = 0x%x\n",
                   req_fw_nowait, ret);
            goto done;
        }
...
#define USB_WAIT_FW_READY  (500)
                ...
                if ((usbRstDev_ret == 0) ||
                    (usbRstDev_ret == -ENODEV) || //expected since chip re-enumerates
                    (usbRstDev_ret == -EINVAL)) { //expected if USB FW detaches first
                    PRINTM(MMSG, "usb_reset_device() successful.\n");
                } else {
                    PRINTM(MERROR, "usb_reset_device() failed with error code =%d\n", usbRstDev_ret);
                    ret = DRIVER_STATUS_FAILURE;
                }
            } else {
