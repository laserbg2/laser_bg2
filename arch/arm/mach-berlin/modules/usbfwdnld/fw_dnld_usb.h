...
driver_status driver_write_data_sync(driver_handle * handle, driver_buffer * pmbuf,
                                 t_u8 ep, t_u32 timeout);
/** Read data blocks from device */
driver_status driver_read_data_sync(driver_handle * handle, driver_buffer * pmbuf,
                                t_u8 ep, t_u32 timeout);
/** Free Tx/Rx urb, skb and Rx buffer */
void driver_usb_free(struct usb_card_rec *cardp);
/** Check chip revision */
driver_status driver_check_chip_revision(driver_handle *handle, t_u32 *usb_chip_rev);
