...
#define DEBUG_DUMP_CTRL_REG               0xE2
#define DEBUG_DUMP_START_REG              0xE3
	...
	for (tries = 0; tries < MAX_POLL_TRIES; tries++) {
		...
		if (ctrl_data != DEBUG_HOST_READY) {
			...
			sdio_writeb(((struct sdio_mmc_card *)phandle->card)->
				    func, DEBUG_HOST_READY, DEBUG_DUMP_CTRL_REG,
				    &ret);
			if (ret) {
				PRINTM(MERROR, "SDIO Write ERR\n");
				return RDWR_STATUS_FAILURE;
...
woal_dump_firmware_info(moal_handle * phandle)
{
	...
	t_u8 read_reg = 0;
	t_u32 memory_size = 0;
	t_u8 filename[MAX_FULL_NAME_LEN];
	mm_segment_t fs;
	loff_t pos;
	t_u8 *end_ptr = NULL;
	...
	for (idx = 0; idx < dump_num; idx++) {
		...
		do {
			...
			if (RDWR_STATUS_DONE == stat) {
				...
				memcpy(filename, name_prefix,
				       strlen(name_prefix));
				strcat(filename,
				       mem_type_mapping_tbl[idx].mem_name);
				moal_get_system_time(phandle, &sec, &usec);
				mem_type_mapping_tbl[idx].pfile_mem =
					filp_open(filename, O_CREAT | O_RDWR,
						  0644);
				if (IS_ERR(mem_type_mapping_tbl[idx].pfile_mem)) {
					PRINTM(MMSG,
					       "Create %s file failed at %s, try to open another dir /var/\n",
					       mem_type_mapping_tbl[idx].
					       mem_name, filename);
					memset(filename, 0, sizeof(filename));
					sprintf(filename, "%s%s", "/var/file_",
						mem_type_mapping_tbl[idx].
						mem_name);
					mem_type_mapping_tbl[idx].pfile_mem =
						filp_open(filename,
				...
				if (!IS_ERR
				    (mem_type_mapping_tbl[idx].pfile_mem)) {
					PRINTM(MMSG,
					       "Now Start to save the output : %u.%06u, please wait...\n",
					       sec, usec);
					fs = get_fs();
					set_fs(KERNEL_DS);
					pos = 0;
					vfs_write(mem_type_mapping_tbl[idx].
						  pfile_mem,
						  mem_type_mapping_tbl[idx].
						  mem_Ptr, memory_size, &pos);
					filp_close(mem_type_mapping_tbl[idx].
						   pfile_mem, NULL);
					set_fs(fs);
					PRINTM(MMSG,
