...
process_vhtcfg(int argc, char *argv[])
{
	...
	num = *(cmd->buf);
	vhtcfg = (struct eth_priv_vhtcfg *)(cmd->buf + 1);
	if (argc == 5) {
		/* GET operation */
...
process_ht_stream_cfg(int argc, char *argv[])
{
	...
	/* Process result */
	memcpy(&mode, cmd->buf, sizeof(mode));
...
process_atim_window(int argc, char *argv[])
{
	...
	/* GET operation */
	if (argc == 3) {
		memcpy(&atim_window, cmd->buf, sizeof(atim_window));
...
process_noa_cfg(int argc, char *argv[])
{
	...
	if (argc == 3) {
		noa_cfg = (mlan_ds_wifi_direct_config *) (cmd->buf);
		if (!noa_cfg) {
...
process_coex_rx_winsize(int argc, char *argv[])
{
	...
	if (argc == 3) {
		memcpy(&coex_rx_winsize, cmd->buf, sizeof(coex_rx_winsize));
