 ...
 */
inline t_u8
wlan_get_port_index(t_u32 port)
{
	const t_u32 usb_tx_ports[] = { MLAN_USB_EP_DATA, MLAN_USB_EP_DATA_CH2 };
	t_u8 i;
	for (i = 0; i < MAX_USB_TX_PORT_NUM; i++) {
		if (port == usb_tx_ports[i]) {
			return i;
		}
	}
	return 0;
...
{
	const t_u32 usb_tx_ports[] = { MLAN_USB_EP_DATA, MLAN_USB_EP_DATA_CH2 };
	t_u32 active_port = MLAN_USB_EP_DATA;
	int i;
	/* MC is enabled */
		...
		for (i = 0; i < MIN(pmadapter->priv_num, MLAN_MAX_BSS_NUM); i++) {
			if (pmadapter->priv[i]) {
				...
				pmadapter->priv[i]->port_index =
					wlan_get_port_index(active_port);
			}
		}
		for (i = 0; i < MAX_USB_TX_PORT_NUM; i++) {
			if (active_port == usb_tx_ports[i])
				pmadapter->usb_port_status[i] = MFALSE;
			else
